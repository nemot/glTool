Ext.define('Gl.controller.Requests', {
  ...
  showRequestWindow: function(){
    ...
    if(rec) {
      window.open('/requests/'+rec.get('id'),'Заявка №'+rec.get('id'), 'width=1100,height=730,location=no,menubar=no')
    } else {
      window.open('/requests/new','Новая заявка', 'width=1000,height=550,location=no,menubar=no')
    }
